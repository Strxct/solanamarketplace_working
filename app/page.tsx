  ...
  useEffect(() => {
    const fetchCollections = async () => {
      try {
        ...
        // Sort collections by createdAt descending (newest first)
        const latest = data.collections
          .sort(
            (a: any, b: any) =>
              new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
          )
          .slice(0, 4); // Take top 4 newest collections
          ...
          latest.map(async (collection: any) => {
            try {
              const metadataResponse = await fetch(
                collection.collectionMetadataUri.replace(
                  "ipfs://",
              ...
              console.error(
                `Failed to fetch metadata for ${collection.collectionMint}`,
                err
              );
              return {
                ...collection,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight">
              Create and Collect{" "}
              <span className="text-primary">Solana NFTs</span> with Ease
            </h1>
            <p className="text-muted-foreground text-lg">
              WorldNFT Solana Drops is a modern platform for creating, minting,
              and collecting NFTs on the Solana blockchain.
            </p>
                ...
                <div className="grid grid-cols-2 gap-4 h-full">
                  {latestCollections.length > 0
                    ...
                    : [1, 2, 3, 4].map((i) => (
                        <div
                          key={i}
                          className="aspect-square rounded-lg bg-muted/50 shimmer"
                        ></div>
                      ))}
                </div>
              </div>
        ...
        <div className="container">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold mb-4">
              Why Choose WorldNFT Solana Drops?
            </h2>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              Our platform offers a seamless experience for creators and
              collectors alike.
            </p>
          </motion.div>
            ...
            {[
              {
                ...
                description:
                  "Leverage Solana's high-speed, low-cost blockchain for efficient NFT transactions.",
              },
              {
                icon: <Shield className="h-10 w-10 text-primary" />,
                title: "Secure & Reliable",
                description:
                  "Built with industry-leading security practices to protect your digital assets.",
              },
              {
                icon: <Sparkles className="h-10 w-10 text-primary" />,
                title: "Creator-Friendly",
                description:
                  "Intuitive tools to help you create, mint, and manage your NFT collections.",
              },
            ].map((feature, index) => (
              <motion.div
                key={index}
        ...
        >
          <div className="rounded-xl bg-card p-8 md:p-12 text-center">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto mb-8">
              Join WorldNFT Solana Drops today and discover the exciting world
              of Solana NFTs.
            </p>
            <Button
              asChild
